...
export const SettingsTab: React.FC<SettingsTabProps> = ({ 
  ...
  const permissionDefinitions = [
    { key: 'canAccessPOS', labelAr: 'الكاشير والبيع السريع (POS)', labelEn: 'POS & Cashier', descAr: 'إصدار الفواتير وطباعة الوصولات والخصومات', icon: Zap, getVal: (p?: UserPermissions) => p?.canAccessPOS ?? true },
    { key: 'canManageProducts', labelAr: 'الأصناف والمنتجات والأسعار', labelEn: 'Products & Pricing', descAr: 'إضافة وتعديل المنتجات وأسعار المفرد والجملة', icon: Tag, getVal: (p?: UserPermissions) => p?.canManageProducts ?? false },
    { key: 'canAccessDashboard', labelAr: 'لوحة التحكم الرئيسية والمالية', labelEn: 'Main Overview Dashboard', descAr: 'رؤية إجمالي أرباح اليوم والمبيعات والصندوق', icon: Sliders, getVal: (p?: UserPermissions) => p?.canAccessDashboard !== false },
    { key: 'canViewReports', labelAr: 'تقارير الأرباح والمبيعات', labelEn: 'Profit & Sales Reports', descAr: 'تصدير التقارير وتحليلات الأرباح اليومية والشهريّة', icon: FileText, getVal: (p?: UserPermissions) => p?.canViewReports ?? false },
    { key: 'canViewAnalytics', labelAr: 'تحليلات الأداء والذكاء الاصطناعي', labelEn: 'AI Analytics & Forecast', descAr: 'استشارات الذكاء الاصطناعي وتحليل حركة المخزن', icon: Cpu, getVal: (p?: UserPermissions) => (p?.canViewAnalytics ?? p?.canViewReports) ?? false },
    { key: 'canManagePurchases', labelAr: 'فواتير الشراء والتجهيز', labelEn: 'Purchase Invoices', descAr: 'إدخال وجدول فواتير التجهيز والمشتريات والديون', icon: Layers, getVal: (p?: UserPermissions) => (p?.canManagePurchases ?? p?.canManageProducts) ?? false },
    { key: 'canManageInventoryAudit', labelAr: 'جرد وتفتيش المخزون', labelEn: 'Stock Audit & Inventory', descAr: 'فحص نواقص المواد وجرد كميات الرفوف والباركود', icon: Barcode, getVal: (p?: UserPermissions) => (p?.canManageInventoryAudit ?? p?.canManageProducts) ?? false },
    { key: 'canManageSuppliers', labelAr: 'سجل الموردين والشركات', labelEn: 'Suppliers & Vendors', descAr: 'إدارة شركات التجهيز والمندوبين والحسابات الدائنة', icon: Radio, getVal: (p?: UserPermissions) => p?.canManageSuppliers ?? false },
    { key: 'canManageCustomers', labelAr: 'إدارة العملاء والزبائن والديون', labelEn: 'Customers & Loyalty', descAr: 'تسجيل الديون والعملاء وبرنامج النقاط والولاء', icon: Users, getVal: (p?: UserPermissions) => p?.canManageCustomers ?? true },
    { key: 'canViewInvoices', labelAr: 'سجل الفواتير والمقبوضات', labelEn: 'Saved Sales Invoices', descAr: 'استعراض الفواتير السابقة وإلغاء المبيعات', icon: CheckCircle2, getVal: (p?: UserPermissions) => (p?.canViewInvoices ?? p?.canManageOrders) ?? false },
    { key: 'canManageOrders', labelAr: 'طلبات الشحنات ورسائل السوق', labelEn: 'Market Orders', descAr: 'متابعة الشحنات والطلبات الواردة للمتجر', icon: Globe, getVal: (p?: UserPermissions) => p?.canManageOrders ?? false },
    { key: 'canManageSettings', labelAr: 'إدارة إعدادات المنظومة وصلاحيات الحسابات', labelEn: 'System Settings & Users', descAr: 'التحكم بإعدادات المتجر وإنشاء وتعديل الحسابات', icon: ShieldAlert, getVal: (p?: UserPermissions) => p?.canManageSettings ?? false },
  ];
      ...
      <div className="bg-[#10192D] p-5 rounded-3xl border border-blue-500/20 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            {isAr ? 'إعدادات المتجر وإنشاء وتعديل الحسابات' : 'Store Settings & User Management'}
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            {isAr ? 'تخصيص بيانات الفواتير الضريبية، العملة، وإنشاء وتعديل ملفات الكاشير والمشرفين مع تحديد الصلاحيات' : 'Configure VAT tax rates, store metadata, create user profiles, and assign permissions'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <span>{isAr ? '⌨️ اختصارات لوحة المفاتيح' : '⌨️ POS Keyboard Shortcuts'}</span>
          </button>
          ...
          >
            ...
            <span>{isAr ? '➕ إنشاء حساب جديد وتعديل الملف' : 'Create New Account'}</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-3 gap-2">
          <div>
            <h3 className="text-sm font-bold text-cyan-400 flex items-center gap-2">
              ...
              {isAr ? 'قائمة حسابات وموظفي المنظومة' : 'System Users & Permission Management'}
            </h3>
            <p className="text-[11px] text-slate-400 mt-0.5">
              {isAr ? 'انقر على "تعديل الملف والصلاحيات" للتحكم بالدخول والصلاحيات' : 'Click "Edit Profile & Permissions" to update user credentials and rights'}
            </p>
          </div>
          <span className="text-xs text-slate-300 bg-slate-800 px-3 py-1 rounded-full border border-slate-700 font-bold">
            {userAccounts.length} {isAr ? 'حسابات مسجلة' : 'Accounts'}
          </span>
        </div>
          ...
          <table className="w-full text-left rtl:text-right text-xs">
            <thead>
              <tr className="border-b border-slate-800 text-slate-400 font-semibold uppercase text-[10px] bg-[#0B1120]/80">
                <th className="py-3 px-3">{isAr ? 'الملف الشخصي والاسم' : 'User Profile'}</th>
                <th className="py-3 px-3">{isAr ? 'بيانات الدخول (اسم المستخدم/الكلمة)' : 'Credentials'}</th>
                <th className="py-3 px-3">{isAr ? 'التخصص والرتبة' : 'Specialization & Role'}</th>
                <th className="py-3 px-3 text-center">{isAr ? 'الصلاحيات الممنوحة' : 'Granted Permissions'}</th>
              ...
              {userAccounts.map((usr) => (
                <tr key={usr.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3 px-3 font-mono text-slate-300">
                    ...
                    <div className="text-[10px] text-slate-400 font-mono">كلمة السر: ••••••••</div>
                  </td>
                  ...
                  <td className="py-3 px-3">
                    <span className={`inline-block px-2.5 py-1 rounded-xl text-[10px] font-bold border ${
                      ...
                      {usr.role === 'Admin' ? (isAr ? 'مدير عام المنظومة' : 'Super Admin') :
                       usr.role === 'Manager' ? (isAr ? 'مدير فرع ومخزن' : 'Branch Manager') :
                       (isAr ? 'كاشير مبيعات' : 'Sales Cashier')}
                    </span>
                    <p className="text-[10px] text-slate-400 mt-0.5 truncate max-w-[150px]">
                      {usr.specialization || (usr.role === 'Admin' ? 'مدير المشرفين' : 'مسؤول المبيعات')}
                    </p>
                  </td>
                  ...
                  <td className="py-3 px-3 text-center">
                    ...
                    >
                      {usr.active ? (isAr ? 'مفعل ✓' : 'Active') : (isAr ? 'معطل ✕' : 'Disabled')}
                    </button>
                  </td>
                  ...
                  <td className="py-3 px-3 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      <button
                        ...
                        title={isAr ? 'تعديل الملف والصلاحيات' : 'Edit Profile & Permissions'}
                      >
                        <Edit3 className="w-3 h-3" />
                        <span>{isAr ? 'تعديل الملف' : 'Edit'}</span>
                      </button>
        ...
        <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-4 gap-3">
          <div>
            <h3 className="text-base font-black text-cyan-300 flex items-center gap-2">
              ...
              <span>{isAr ? 'قسم تعديل وتخصيص صلاحيات المستخدمين بالتفصيل (Toggle Switches)' : 'Granular User Permissions Manager'}</span>
            </h3>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 font-bold">{isAr ? 'اختر الحساب:' : 'Select Account:'}</span>
            <select
            ...
            >
              {userAccounts.map((u) => (
                <option key={u.id} value={u.id}>
                  {u.fullName} (@{u.username}) - {u.role === 'Admin' ? (isAr ? 'مدير عام' : 'Admin') : u.role === 'Manager' ? (isAr ? 'مدير' : 'Manager') : (isAr ? 'كاشير' : 'Cashier')}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800 gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-100 flex items-center gap-1.5">
                    ...
                    <span className={`px-2 py-0.5 rounded-md text-[10px] ${
                      ...
                      {selectedUser.role === 'Admin' ? (isAr ? 'مدير عام' : 'Super Admin') : selectedUser.role === 'Manager' ? (isAr ? 'مدير فرع' : 'Manager') : (isAr ? 'كاشير مبيعات' : 'Sales Cashier')}
                    </span>
                  </p>
                  <p className="text-[10px] text-slate-400 font-mono mt-0.5">اسم المستخدم: @{selectedUser.username}</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {isAr ? '⚡ تفعيل كافة القوائم' : 'Enable All'}
                </button>
                ...
                >
                  {isAr ? '🛒 صلاحيات الكاشير الموصى بها (POS والعملاء فقط)' : 'Recommended Cashier Preset'}
                </button>
                ...
                >
                  {isAr ? '🔒 تعطيل كافة القوائم' : 'Disable All'}
                </button>
              </div>
            </div>
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">{def.descAr}</p>
                      </div>
                    </div>
      ...
      <div className="cyber-card p-6 rounded-3xl border border-blue-500/20 space-y-6 max-w-3xl">
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-r from-cyan-950/60 via-blue-950/50 to-indigo-950/60 border border-cyan-500/30 flex flex-wrap items-center justify-between gap-3 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-xs font-black text-white">
                {isAr ? 'اختصارات الكيبورد لواجهة البيع (POS Hotkeys)' : 'POS Keyboard Hotkeys'}
              </h4>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {isAr ? 'تخصيص المفاتيح مثل F1، F2، F3، F4 لإتمام الدفع، فتح نافذة جديدة، ومسح الباركود بسرعة' : 'Customize hotkeys for fast checkout, new window, and barcode scan focus'}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <span>{isAr ? 'التحكم والتخصيص' : 'Customize Hotkeys'}</span>
          </button>
        </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-right rtl:text-right ltr:text-left">
                  ...
                  <span className="text-[10px] text-slate-400 font-normal">
                    {isAr ? 'مظهر داكن مريح للعينين ومناسب لبيئة الماركيت والليلي' : 'Dark high-contrast theme for comfortable usage'}
                  </span>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-right rtl:text-right ltr:text-left">
                  ...
                  <span className="text-[10px] text-slate-400 font-normal">
                    {isAr ? 'مظهر ناصع عالي الوضوح للبيئات المضاءة نهاراً' : 'Clean bright high-visibility theme'}
                  </span>
                </div>
              </div>
        ...
        <div className="space-y-4 pt-4 border-t border-slate-800">
          <h3 className="text-sm font-bold text-cyan-400 border-b border-slate-800 pb-2 flex items-center gap-2">
            ...
            {settings.language === 'ku' ? 'زانیاری و ناوی کۆگا' : isAr ? 'بيانات الماركيت الرئيسية' : 'Supermarket Branding'}
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            <div>
              <label className="text-slate-400 mb-1 block">
                {settings.language === 'ku' ? 'ناوی کۆگا (کوردی)' : isAr ? 'اسم المتجر (بالعربية)' : 'Store Name (AR)'}
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="text-slate-400 mb-1 block">
                {settings.language === 'ku' ? 'ناوی کۆگا (ئینگلیزی)' : isAr ? 'اسم المتجر (English)' : 'Store Name (EN)'}
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="text-slate-400 mb-1 block">
                {settings.language === 'ku' ? 'ناوی کۆگا (کوردی / کوردی)' : 'اسم المتجر (کوردی)'}
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="text-slate-400 mb-1 block">
                {settings.language === 'ku' ? 'ژمارەی تلفۆن' : isAr ? 'رقم الهاتف' : 'Phone'}
              </label>
              <input
                type="text"
        ...
        <div className="space-y-4 pt-4 border-t border-slate-800">
          <h3 className="text-sm font-bold text-cyan-400 border-b border-slate-800 pb-2 flex items-center gap-2">
            ...
            {isAr ? 'إعدادات الضريبة والعملة' : 'Currency & VAT Tax Rates'}
          </h3>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            <div>
              <label className="text-slate-400 mb-1 block">{isAr ? 'العملة المعروضة' : 'Currency Symbol'}</label>
              <select
              ...
              >
                <option value="د.ع">د.ع IQD (دينار عراقي)</option>
                <option value="$">$ USD (دولار)</option>
                <option value="ر.س">ر.س SAR (ريال سعودي)</option>
                <option value="ج.م">ج.م EGP (جنيه مصري)</option>
                <option value="€">€ EUR (يورو)</option>
              </select>
            </div>

            <div>
              <label className="text-slate-400 mb-1 block">{isAr ? 'نسبة القيمة المضافة (%)' : 'VAT Tax Rate (%)'}</label>
              <input
                type="number"
            ...
            <div>
              <label className="text-slate-400 mb-1 block">{isAr ? 'حد انخفاض المخزون الافتراضي' : 'Low Stock Default'}</label>
              <input
                type="number"
        ...
        <div className="space-y-5 pt-4 border-t border-slate-800">
          <div className="flex flex-wrap items-center justify-between border-b border-slate-800 pb-3 gap-2">
            <div>
              <h3 className="text-sm font-bold text-cyan-400 flex items-center gap-2">
                ...
                <span>{isAr ? 'مركز ربط وإدارة كافة أنواع الطابعات (المكتبية الكبيرة A4، الحرارية، وملصقات الأسعار والباركود)' : 'All Printer Connection & Hardware Management Hub'}</span>
              </h3>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {isAr ? 'ربط كافة الطابعات الموصولة باللابتوب/الكمبيوتر عن طريق USB، الشبكة اللاسلكية LAN، البلوتوث، أو مشغل طابعات الويندوز المباشر' : 'Connect & link any printer attached to your laptop via USB, Network LAN, Bluetooth, or OS Spooler'}
              </p>
            </div>
            ...
            >
              ...
              <span>{isScanningPrinters ? (isAr ? 'جاري فحص الطابعات...' : 'Scanning...') : (isAr ? '⚡ فحص واكتشاف الطابعات المربوطة بالكمبيوتر' : 'Scan Connected Printers')}</span>
            </button>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-950/70 via-[#0B1120] to-cyan-950/70 border border-amber-500/40 space-y-3 shadow-lg">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-xs sm:text-sm font-extrabold text-white flex items-center gap-2">
                    <span>{isAr ? 'الطباعة الحرارية المباشرة (ESC/POS Web Serial Direct Print)' : 'Direct ESC/POS Thermal Printing (Web Serial)'}</span>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-mono border border-emerald-500/30 font-bold">
                      {isWebSerialSupported() ? 'مدعوم ⚡' : 'غير مدعوم'}
                    </span>
                  </h4>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    setPrinterScanMessage(isAr ? 'جاري اختبار الاتصال بالطابعة الحرارية المباشرة...' : 'Testing connection to direct thermal printer...');
                    ...
                    setPrinterScanMessage(isAr ? '✅ نجح الاتصال وتمت الطباعة التجريبية بنجاح على الطابعة الحرارية!' : '✅ Web Serial Direct Thermal Print Test Successful!');
                  } catch (e: any) {
                    setPrinterScanMessage(isAr ? `تنبيه: ${e.message || 'تعذر الاتصال بالطابعة الحرارية'}` : `Error: ${e.message}`);
                  }
                }}
              ...
              >
                ...
                <span>{isAr ? 'ربط واختبار الطابعة الحرارية المباشرة (USB/Serial)' : 'Pair & Test Thermal Printer'}</span>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-[#0B1120] p-4 rounded-2xl border border-blue-500/20">
            <div>
              <label className="text-slate-300 font-bold mb-1.5 flex items-center gap-2 text-xs">
                ...
                <span>{isAr ? '1. الطابعة الرئيسية المربوطة للإيصالات والفواتير:' : '1. Linked Main Invoice Printer:'}</span>
              </label>
              <select
            ...
            <div>
              <label className="text-slate-300 font-bold mb-1.5 flex items-center gap-2 text-xs">
                ...
                <span>{isAr ? '2. طابعة ملصقات الأسعار والباركود المربوطة:' : '2. Linked Price Label & Barcode Printer:'}</span>
              </label>
              <select
          ...
          <div>
            <label className="text-xs font-bold text-slate-300 mb-2 block">
              {isAr ? 'اختر قالب ونوع الطابعة النشطة في المنظومة:' : 'Select Default Printer Type Profile:'}
            </label>
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">🧾 80mm حرارية POS</span>
                  </div>
                  {(settings.printerType || 'thermal80mm') === 'thermal80mm' && <Check className="w-4 h-4 text-cyan-400" />}
                </div>
                <p className="text-[10px] text-slate-400 leading-snug">
                  {isAr ? 'إيصال حراري ضيق للكاشير السريع (Epson, Xprinter, Bixolon)' : 'Thermal roll for fast checkout'}
                </p>
              </button>
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">🖨️ A4 مكتبية كبيرة</span>
                  </div>
                  {settings.printerType === 'a4' && <Check className="w-4 h-4 text-blue-400" />}
                </div>
                <p className="text-[10px] text-slate-400 leading-snug">
                  {isAr ? 'طابعة ورقية عادية كبيرة (HP LaserJet, Canon, Brother)' : 'Full page invoice document'}
                </p>
              </button>
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">🏷️ ملصقات الأسعار</span>
                  </div>
                  {settings.printerType === 'label_barcode' && <Check className="w-4 h-4 text-amber-400" />}
                </div>
                <p className="text-[10px] text-slate-400 leading-snug">
                  {isAr ? 'طباعة أسعار وباركود المواد (Xprinter, Zebra, TSC)' : 'Price tags & barcode sticky labels'}
                </p>
              </button>
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white">📄 A5 نصف ورقة</span>
                  </div>
                  {settings.printerType === 'a5' && <Check className="w-4 h-4 text-purple-400" />}
                </div>
                <p className="text-[10px] text-slate-400 leading-snug">
                  {isAr ? 'فواتير جملة وتوصيل مقاس نصف ورقة A5' : 'Compact A5 invoice format'}
                </p>
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-[#0B1120] p-4 rounded-2xl border border-slate-800 text-xs">
            ...
            <div>
              <label className="text-slate-400 mb-1 block font-bold flex items-center gap-1">
                ...
                <span>{isAr ? 'طريقة الربط والاتصال:' : 'Connection Method:'}</span>
              </label>
              <select
              ...
              >
                <option value="system">💻 طابعة الويندوز/اللابتوب الافتراضية (OS Spooler)</option>
                <option value="usb">🔌 وصلة كابل USB المباشر (Direct USB Port)</option>
                <option value="network">🌐 طابعة شبكة لاسلكية (LAN / Network IP)</option>
                <option value="bluetooth">📶 اتصال بلوتوث لاسلكي (Bluetooth Thermal)</option>
              </select>
            </div>
            ...
            <div>
              <label className="text-slate-400 mb-1 block font-bold flex items-center gap-1">
                ...
                <span>{isAr ? 'عنوان الشبكة IP / المنفذ:' : 'IP Address / Port:'}</span>
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="text-slate-400 mb-1 block font-bold flex items-center gap-1">
                ...
                <span>{isAr ? 'مقاس الورق والملصق:' : 'Paper / Label Size:'}</span>
              </label>
              <select
              ...
              >
                <option value="80mm">80mm (شريط حراري عريض)</option>
                <option value="58mm">58mm (شريط حراري ضيق)</option>
                <option value="A4">A4 (ورقة مكتبية كاملة)</option>
                <option value="A5">A5 (نصف ورقة)</option>
                <option value="50x30mm">50x30 mm (ملصق أسعار وباركود)</option>
                <option value="40x20mm">40x20 mm (ملصق باركود مصغر)</option>
              </select>
            </div>
            ...
            <div>
              <label className="text-slate-400 mb-1 block font-bold flex items-center gap-1">
                ...
                <span>{isAr ? 'عدد النسخ المطبوعة تلقائياً:' : 'Auto Printed Copies:'}</span>
              </label>
              <input
                type="number"
                min={1}
          ...
          <div className="bg-gradient-to-r from-slate-900 via-blue-950/60 to-slate-900 p-4 rounded-2xl border border-cyan-500/30 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h4 className="text-xs font-black text-white flex items-center gap-2">
                ...
                <span>{isAr ? 'اختبار طباعة مباشر لتأكيد ربط الطابعة بالبرنامج:' : 'Direct Printer Connection Test Suite:'}</span>
              </h4>
              <p className="text-[11px] text-slate-400 mt-0.5">
                {isAr ? 'انقر على أي من الأزرار التالية لإرسال أمر طباعة تجريبي واختبار جودة الورق والخطوط' : 'Click any button to trigger an instant test print to verify your connected hardware'}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                <span>{isAr ? '🧾 تجربة إيصال POS' : 'Test Receipt'}</span>
              </button>
              ...
              >
                ...
                <span>{isAr ? '🏷️ تجربة ملصق سعر وباركود' : 'Test Price Tag'}</span>
              </button>
              ...
              >
                ...
                <span>{isAr ? '📄 تجربة تقرير A4' : 'Test A4 Page'}</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-slate-800">
          <h3 className="text-sm font-bold text-cyan-400 border-b border-slate-800 pb-2">
            {isAr ? 'ترويسة وتذييل الفاتورة المطبوعة' : 'Receipt Messages'}
          </h3>

          <div className="space-y-3 text-xs">
            <div>
              <label className="text-slate-400 mb-1 block">{isAr ? 'رسالة الترحيب أعلى الفاتورة' : 'Header Message'}</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-slate-400 mb-1 block">{isAr ? 'رسالة الشكر وسياسة الترجيع في الأسفل' : 'Footer Return Policy Message'}</label>
              <input
                type="text"
          ...
          <div className="p-3.5 rounded-2xl bg-[#08101E] border border-emerald-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
            ...
            <div className="flex flex-wrap items-center gap-2 shrink-0">
                  ...
                  try {
                    if (file.name.endsWith('.xlsx') || file.name.endsWith('.xls')) {
                      ...
                      if (excelParsed.products && excelParsed.products.length > 0) {
                        ...
                        setImportStatus(isAr ? `✅ تم استيراد وترتيب ${excelParsed.products.length} مادة وإضافتها للمخزن بنجاح!` : `✅ Successfully imported and arranged ${excelParsed.products.length} products into inventory!`);
                      ...
                      reader.onload = (event) => {
                        try {
                          ...
                          setImportStatus(isKu ? 'زانیارییەکان بە سەرکەوتوویی گەڕێنرانەوە!' : isAr ? `✅ تم استعادة وترتيب كافة البيانات (${prodsCount} مادة) بنجاح!` : 'Data restored successfully!');
                          setTimeout(() => setImportStatus(''), 5000);
                        } catch (err) {
                          alert(isKu ? 'پەڕگەی JSON دروست نییە!' : isAr ? 'ملف النسخة الاحتياطية غير صالح!' : 'Invalid backup JSON file!');
                        }
                      };
              ...
              >
                ...
                <span>{isAr ? '📊 تصدير نسخة احتياطية شاملة (Excel .xlsx)' : 'Export Full Store Backup (Excel .xlsx)'}</span>
              </button>
              ...
              >
                ...
                <span>{isAr ? 'تصدير جدول المواد (Excel)' : 'Export Products List (Excel)'}</span>
              </button>
              ...
              >
                ...
                <span>{isAr ? 'تصدير بيانات JSON' : 'Export JSON Data'}</span>
              </button>
              ...
              >
                ...
                <span>{isAr ? 'استيراد ملف (Excel / JSON)' : 'Import (Excel / JSON)'}</span>
              </button>
            </div>
          </div>
        </div>
