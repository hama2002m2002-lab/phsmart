...
export const ShiftReportModal: React.FC<ShiftReportModalProps> = ({
  ...
  }, [dateFilterMode, selectedCustomDate, lang, isAr]);
      ...
      <div className="cyber-card w-full max-w-6xl max-h-[92vh] rounded-3xl border border-cyan-500/40 bg-[#0A1220] text-slate-100 flex flex-col overflow-hidden shadow-[0_0_60px_rgba(6,182,212,0.25)] relative print:hidden">
        ...
        <div className="p-4 sm:p-5 border-b border-cyan-500/20 flex flex-wrap items-center justify-between gap-3 bg-[#0F192C]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {isAr ? `ملخص المبيعات، عدد الفواتير، وحركة الخزنة للفترة: ${activeDisplayDateText}` : `Shift overview, sales breakdown & cash movements: ${activeDisplayDateText}`}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex bg-[#070D18] p-1 rounded-xl border border-slate-700 text-xs">
              ...
              >
                {isAr ? 'وصل حراري 80mm' : 'Thermal 80mm'}
              </button>
              <button
                type="button"
            ...
            {onOpenSalesReturn && (
              <button
                ...
                title={isAr ? 'إرجاع مواد واسترجاع المبيعات' : 'Sales Return'}
              >
                <RotateCcw className="w-4 h-4 text-rose-100" />
                <span>{isAr ? 'إرجاع مواد' : 'Sales Return'}</span>
              </button>
            )}
            ...
            >
              ...
              <span>{isAr ? 'طباعة تقرير الوردية' : 'Print Shift Report'}</span>
            </button>
          ...
          <div className="p-3.5 rounded-2xl bg-[#0B1528] border border-cyan-500/30 flex flex-wrap items-center justify-between gap-3 shadow-lg">
            <div className="flex flex-wrap items-center gap-2 text-xs">
              <span className="font-bold text-cyan-300 flex items-center gap-1.5 ml-1">
                ...
                <span>{isAr ? 'تحديد تاريخ الوردية:' : 'Select Shift Date:'}</span>
              </span>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={isAr ? 'بحث برقم الوصل، العميل...' : 'Search invoice, customer...'}
              ...
              {uniqueCashiers.length > 0 && (
                ...
                >
                  <option value="all">{isAr ? 'جميع الكاشيرين' : 'All Cashiers'}</option>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-950/80 via-[#0C1B17] to-teal-950/70 border border-emerald-500/40 shadow-lg relative overflow-hidden">
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                {isAr ? `من إجمالي مبيعات: ${settings.currencySymbol}${formatNumber(grossSales)}` : `Gross: ${settings.currencySymbol}${formatNumber(grossSales)}`}
              </p>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-[#0F192B] border border-cyan-500/30 shadow-md">
              ...
              <div className="text-2xl font-black text-cyan-400 font-mono">
                {invoiceCount} <span className="text-xs text-slate-400 font-sans">{isAr ? 'فاتورة' : 'invoices'}</span>
              </div>
              <p className="text-[10px] text-slate-400 mt-1">
                {isAr ? `إجمالي القطع المباعة: ${totalItemsSold} قطعة` : `Total items: ${totalItemsSold}`}
              </p>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-[#111C2C] border border-blue-500/30 shadow-md">
              <div className="flex items-center justify-between text-xs text-blue-300 font-bold mb-1">
                <span>{isAr ? 'النقد المتوقع بالخزنة' : 'Expected Cash in Safe'}</span>
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                {isAr ? 'يشمل الرصيد الافتتاحي والمبيعات النقدية' : 'Includes float & cash sales'}
              </p>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-[#1C111A] border border-rose-500/30 shadow-md">
              <div className="flex items-center justify-between text-xs text-rose-300 font-bold mb-1">
                <span>{isAr ? 'المرتجعات والخصومات' : 'Refunds & Discounts'}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-[#0D1525] border border-slate-800 space-y-4">
              <h3 className="text-xs font-bold text-slate-200 flex items-center gap-2 border-b border-slate-800 pb-2.5">
                ...
                <span>{isAr ? 'توزيع المبيعات حسب طريقة الدفع:' : 'Sales Breakdown by Payment Method:'}</span>
              </h3>
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 rounded-xl bg-[#080E1B] border border-emerald-500/20 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-200">{isAr ? 'مبيعات نقدية (Cash)' : 'Cash Sales'}</p>
                      <p className="text-[10px] text-slate-400">{cashSalesList.length} {isAr ? 'فاتورة' : 'invoices'}</p>
                    </div>
                  </div>
                ...
                <div className="p-3 rounded-xl bg-[#080E1B] border border-blue-500/20 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-200">{isAr ? 'بطاقة/شبكة (Card POS)' : 'Card Sales'}</p>
                      <p className="text-[10px] text-slate-400">{cardSalesList.length} {isAr ? 'فاتورة' : 'invoices'}</p>
                    </div>
                  </div>
                ...
                <div className="p-3 rounded-xl bg-[#080E1B] border border-purple-500/20 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-200">{isAr ? 'دفع ذكي (NFC / Apple Pay)' : 'NFC Mobile Sales'}</p>
                      <p className="text-[10px] text-slate-400">{nfcSalesList.length} {isAr ? 'فاتورة' : 'invoices'}</p>
                    </div>
                  </div>
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-[#0D1525] border border-slate-800 space-y-4">
              <h3 className="text-xs font-bold text-slate-200 flex items-center gap-2 border-b border-slate-800 pb-2.5">
                ...
                <span>{isAr ? 'حركة وتقريب درج النقود (Cash Drawer Audit):' : 'Cash Drawer Reconciliation:'}</span>
              </h3>
              ...
              <div className="space-y-2 text-xs font-sans">
                ...
                <div className="flex justify-between p-2.5 rounded-xl bg-emerald-950/40 border border-emerald-500/40 font-bold text-emerald-300">
                  <span>{isAr ? '(=) الصافي المتوقع بالخزنة:' : '(=) Expected Net Cash:'}</span>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-300 flex items-center justify-between">
              <span className="flex items-center gap-2">
                ...
                <span>{isAr ? `فواتير ومبيعات الفترة المسجلة (${activeDisplayDateText}):` : `Shift Transactions (${activeDisplayDateText}):`}</span>
              </span>
              <span className="text-[11px] font-mono text-cyan-400 bg-cyan-950 px-2.5 py-0.5 rounded-full border border-cyan-500/30 font-bold">
                {filteredSales.length} {isAr ? 'عمليات' : 'Tx'}
              </span>
            </h3>
              ...
              <div className="max-h-72 overflow-y-auto divide-y divide-slate-800/60 custom-scrollbar">
                {filteredSales.length === 0 ? (
                  <div className="p-8 text-center text-slate-500 text-xs font-bold space-y-1">
                    <p>{isAr ? 'لا توجد فواتير مبيعات مسجلة في هذا التاريخ أو للبحث المحدد' : 'No sales recorded for this date or search query.'}</p>
                    <p className="text-[10px] text-slate-600">{isAr ? 'يمكنك استخدام التقويم أعلاه لاختيار أي تاريخ آخر' : 'Use the calendar above to pick any date.'}</p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-300 font-bold">
                            {sale.paymentMethod === 'card' ? (isAr ? 'بطاقة' : 'Card') : sale.paymentMethod === 'nfc' ? 'NFC' : (isAr ? 'كاش' : 'Cash')}
                          </span>
                          <span className="text-[10px] text-slate-400">
                            ({(Array.isArray(sale.items) ? sale.items : (typeof sale.items === 'string' ? JSON.parse(sale.items || '[]') : [])).length} {isAr ? 'عناصر' : 'items'})
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {onOpenSalesReturn && !isRefunded && (
                            <button
                              ...
                              title={isAr ? `إرجاع مواد من الوصل #${sale.invoiceNumber}` : `Return items from receipt #${sale.invoiceNumber}`}
                            >
                              <RotateCcw className="w-3.5 h-3.5 text-rose-400" />
                              <span>{isAr ? 'إرجاع مواد' : 'Return Items'}</span>
                            </button>
                          )}
                        </div>
                      </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-[#0F192C] flex items-center justify-between text-xs">
          <div className="text-slate-400 flex items-center gap-2">
            ...
            <span>{isAr ? `الكاشير المسؤول: ${cashierName}` : `Cashier: ${cashierName}`}</span>
          </div>
        ...
        {printFormat === 'thermal' ? (
          ...
          <div className="w-[78mm] mx-auto text-[11px] font-mono space-y-2 border-b-2 border-dashed pb-4">
            ...
            <div className="text-center space-y-1">
              ...
              <h2 className="text-xs font-black uppercase">*** تقرير ملخص الوردية ***</h2>
            ...
            <div className="space-y-1 border-b border-black pb-2">
              <div className="font-bold border-b border-dotted pb-0.5">--- ملخص المبيعات Sales Summary ---</div>
              <div className="flex justify-between">
                <span>عدد الفواتير Invoices Count:</span>
                <span className="font-bold">{invoiceCount}</span>
              </div>
              <div className="flex justify-between">
                <span>إجمالي القطع المباعة Total Items:</span>
              ...
              <div className="flex justify-between">
                <span>الخصومات Discounts:</span>
                <span>-{settings.currencySymbol}{formatNumber(totalDiscounts)}</span>
              </div>
              <div className="flex justify-between">
                <span>المرتجعات Refunds ({totalReturnedQty} قطعة):</span>
            ...
            <div className="space-y-1 border-b border-black pb-2">
              <div className="font-bold border-b border-dotted pb-0.5">--- طرق الدفع Payments ---</div>
              <div className="flex justify-between">
            ...
            {/* Cash Drawer Audit */}
            <div className="space-y-1 border-b border-black pb-2">
              <div className="font-bold border-b border-dotted pb-0.5">--- حركات الخزنة Cash Drawer ---</div>
              <div className="flex justify-between">
                <span>الرصيد الافتتاحي Opening Float:</span>
                <span>{settings.currencySymbol}{formatNumber(initialFloat)}</span>
              </div>
              <div className="flex justify-between">
                <span>(+) المقبوضات النقدية Cash In:</span>
                <span>+{settings.currencySymbol}{formatNumber(cashSalesTotal)}</span>
              </div>
              <div className="flex justify-between">
                <span>(+) الإيداعات النقدية Deposits:</span>
                <span>+{settings.currencySymbol}{formatNumber(totalDeposits)}</span>
              </div>
              <div className="flex justify-between">
                <span>(-) المرتجعات النقدية Cash Refunds:</span>
                <span>-{settings.currencySymbol}{formatNumber(totalRefunds)}</span>
              </div>
              <div className="flex justify-between">
                <span>(-) السحوبات والمصاريف Withdrawals:</span>
                <span>-{settings.currencySymbol}{formatNumber(totalWithdrawals)}</span>
              </div>
              <div className="flex justify-between font-black text-xs border-t border-black pt-1">
                <span>صافي النقد بالدرج Net Cash:</span>
            ...
            <div className="pt-4 space-y-6 text-[10px] text-center">
              ...
              <p className="text-[9px]">تم طباعة هذا التقرير تلقائياً عبر منظومة إدارة POS</p>
            </div>
          ...
          <div className="max-w-3xl mx-auto p-6 space-y-6 border border-slate-300 rounded-lg">
            ...
            <div className="flex justify-between items-center border-b-2 border-black pb-4">
              <div>
                ...
                <p className="text-xs text-gray-600">{settings.address} | هاتف: {settings.phone}</p>
              </div>
              <div className="text-right rtl:text-left">
                <h2 className="text-lg font-black text-gray-800">تقرير ملخص الوردية اليومي</h2>
                <p className="text-xs text-gray-500">التاريخ: {activeDisplayDateText}</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 text-xs bg-gray-50 p-3 rounded border">
              <div><strong>الكاشير المسؤول:</strong> {cashierName}</div>
              <div><strong>وقت طباعة التقرير:</strong> {new Date().toLocaleTimeString('ar-SA')}</div>
              <div><strong>إجمالي عدد الفواتير:</strong> {invoiceCount} فاتورة</div>
              <div><strong>إجمالي القطع المباعة:</strong> {totalItemsSold} قطعة</div>
            </div>

            <div className="space-y-2">
              <h3 className="font-bold text-sm border-b pb-1">1. البيان المالي العام (Sales Overview)</h3>
            ...
            <div className="space-y-2">
              <h3 className="font-bold text-sm border-b pb-1">2. حركات وتقريب الخزنة والصندوق (Cash Drawer Reconciliation)</h3>
              <table className="w-full text-xs border-collapse border border-gray-300">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="border p-2 text-right">نوع الحركة</th>
                    <th className="border p-2 text-center">المبلغ</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border p-2">الرصيد الافتتاحي للصندوق</td>
                    <td className="border p-2 text-center">{settings.currencySymbol}{formatNumber(initialFloat)}</td>
                  </tr>
                  <tr>
                    <td className="border p-2 text-emerald-700">(+) المقبوضات النقدية من المبيعات</td>
                    <td className="border p-2 text-center text-emerald-700">+{settings.currencySymbol}{formatNumber(cashSalesTotal)}</td>
                  </tr>
                  <tr>
                    <td className="border p-2 text-emerald-700">(+) الإيداعات النقدية اليدوية</td>
                    <td className="border p-2 text-center text-emerald-700">+{settings.currencySymbol}{formatNumber(totalDeposits)}</td>
                  </tr>
                  <tr>
                    <td className="border p-2 text-red-600">(-) المرتجعات النقدية المقتطعة</td>
                    <td className="border p-2 text-center text-red-600">-{settings.currencySymbol}{formatNumber(totalRefunds)}</td>
                  </tr>
                  <tr>
                    <td className="border p-2 text-red-600">(-) السحوبات والمصاريف النقدية</td>
                    <td className="border p-2 text-center text-red-600">-{settings.currencySymbol}{formatNumber(totalWithdrawals)}</td>
                  </tr>
                  <tr className="bg-blue-50 font-bold">
                    <td className="border p-2 text-blue-900">(=) الصافي النهائي الفعلي المتوقع بالخزنة</td>
            ...
            <div className="pt-8 flex justify-between text-xs text-center">
              <div>
                <p className="font-bold">توقيع الكاشير المسؤول</p>
                <div className="mt-8 border-b border-black w-36 mx-auto" />
              </div>
              <div>
                <p className="font-bold">توقيع مشرف المحل / المدير</p>
                <div className="mt-8 border-b border-black w-36 mx-auto" />
              </div>
            </div>
