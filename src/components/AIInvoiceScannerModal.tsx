...
Output strictly valid JSON with this structure:
{
  ...
  "items": []
      ...
      const models = ['gemini-3.1-flash-lite', 'gemini-3.8-flash', 'gemini-3.1-flash-lite-preview', 'gemini-flash-latest', 'gemini-3.1-pro-preview'];
  ...
  const scanSingleImagePage = async (base64Img: string): Promise<ScannedInvoiceData> => {
    ...
    try {
      ...
      const serverErrMsg = errData.error || `Server error: ${response.status}`;

      // If client has custom key and server was 404 (offline/static hosting), try direct client fallback
      if (activeGeminiKey && response.status === 404) {
      ...
      clearTimeout(timeoutId);
      if (activeGeminiKey && (fetchErr?.name === 'TypeError' || String(fetchErr).includes('Failed to fetch'))) {
        try {
          return await callGeminiInvoiceDirectly(activeGeminiKey, optimizedImage);
        } catch (directErr: any) {
  ...
  const processInvoicePdf = async (pdfFile: File) => {
    ...
    try {
      ...
      for (let i = 0; i < pagesDataUrls.length; i++) {
        ...
        let pageResult: ScannedInvoiceData | null = null;
        let lastPageErr: any = null;

        // Resilient retry per page (up to 2 attempts with pause) so multi-page scanning never halts unexpectedly
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            pageResult = await scanSingleImagePage(pagesDataUrls[i]);
            if (pageResult) break;
          } catch (pErr: any) {
            lastPageErr = pErr;
            console.warn(`[AIInvoiceScannerModal] Page ${i + 1} attempt ${attempt} warning:`, pErr);
            if (attempt < 2) {
        ...
        // Throttle 800ms between page requests to protect browser thread & Gemini rate limits
      ...
      if (aggregatedItems.length === 0) {
        throw new Error(t('لم يتم العثور على أدوية أو أسطر في ملف الـ PDF. يرجى التأكد من وضوح الصفحات وإعادة المحاولة.', 'هیچ کاڵایەک نەدۆزرایەوە لە پەڕەکانی ئەم PDFە.', 'No invoice items detected in this PDF. Please ensure pages are clear.'));
      }
